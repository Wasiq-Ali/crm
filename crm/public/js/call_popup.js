		...
		this.dialog = new frappe.ui.Dialog({
			...
			'fields': [{
				...
				'click': () => {
					...
					frappe.xcall('crm.communication.doctype.call_log.call_log.add_call_summary', {
...
$(document).on('app_ready', function () {
	frappe.realtime.on('show_call_popup', call_log => {
		if (!crm.call_popup) {
			crm.call_popup = new CallPopup(call_log);
		} else {
			crm.call_popup.update_call_log(call_log);
			crm.call_popup.dialog.show();
		}
	});
});
